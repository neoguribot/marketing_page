...
def caption(text: str):
    st.markdown(f"<span style='color:{INK_SECONDARY};font-size:0.9em'>{text}</span>", unsafe_allow_html=True)
...
with st.sidebar:
    ...
    with st.container(border=True):
        st.markdown(f"🏢 **업체** &nbsp; {meta.get('업체', '-')}")
        st.markdown(f"🏷️ **업종** &nbsp; {meta.get('업종', '-')}")
        st.markdown(f"📣 **캠페인** &nbsp; {meta.get('캠페인', '-')}")
        st.markdown(f"🎯 **목표** &nbsp; {meta.get('목표', '-')}")
        st.markdown(f"⚙️ **입찰 전략** &nbsp; {meta.get('입찰 전략', '-')}")
        st.markdown(f"📅 **기간** &nbsp; {meta.get('기간', '-')}")

# ---------------- 헤더 ----------------
st.title("📊 소니코리아 검색광고 마케팅 대시보드")
caption(f"데이터 기준: {meta.get('기간', '-')} · 네이버 검색광고 리포트")
...
tab_heat, tab_trend, tab_device, tab_kw = st.tabs(
    ["🗓️ 일별 히트맵", "📈 추이 · 비교", "📱 기기 · 시간대", "🔑 키워드"]
)
...
with tab_heat:
    ...
    caption(
        "색은 지표별(열) 상대값입니다 — 진할수록 그 지표에서 상대적으로 높은 날입니다. "
        "🌱 표시는 자동입찰 학습 기간(초반 7일)입니다. 정확한 값은 히트맵 아래 표를 참고하세요."
    )
...
with tab_trend:
    ...
    caption(
        "두 기간을 각각 선택하면 핵심 지표와 추이를 나란히 비교합니다. "
        "증감률은 기간 A 대비 기간 B 기준입니다."
    )
        ...
        if len(period_a) == 2 and len(period_b) == 2:
            ...
            if sub_a.empty or sub_b.empty:
                ...
                caption(
                    ...
                    f"기간 B: {b_start} ~ {b_end} ({sb['days']}일)"
                )
